...
| title    | str  | Movie title with year in parentheses |                 Toy Story (1995)                |
| genres   | str  | Pipe-separated list of genres        | Adventure\|Animation\|Children\|Comedy\|Fantasy |
...
| rating    | float | Rating from 0.5 to 5.0 (0.5 increments) |                     4.0            |
| timestamp | int   | Unix timestamp (seconds since 1970)     |                    964982703       |
...
Cleaned movies data with additional features.
...
#### `movie_features.csv`
Final feature matrix for movies used in machine learning models.

| Column(s) | Description | Source Library |
|-----------|-------------|----------------|
| `movieId` | Unique movie identifier | - |
| `title` | Movie title | - |
| `year_str` | Year extracted from title | - |
| `genre_count` | Number of genres for this movie | - |
| `rating_count` | Total number of ratings received | - |
| `avg_rating` | Average rating | - |
| `std_rating` | Standard deviation of ratings | - |
| `(no genres listed)` to `Western` | One-hot encoded genre columns (20 columns) | `sklearn.preprocessing.MultiLabelBinarizer` |
| `tag_0` to `tag_49` | TF-IDF features from user tags (50 columns) | `sklearn.feature_extraction.text.TfidfVectorizer` |

**Shape:** (9,742 rows, 77 columns)

#### `user_movie_matrix.csv`
User-movie matrix with raw ratings.
...
#### `user_movie_matrix_norm.csv`
Normalized user-movie matrix (ratings centered by user mean).
